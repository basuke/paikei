...
- `Sources/PaikeiCore/`
  ...
  - `Format/` … `.paikei` の入出力。`Format/Notation/` は値の表記（仕様§4の副露・
    §5の河）、`Format/Stream/` は仕様§8とMJAI方言
  - `Bot/` … 局面から手を決める打ち手
...
- **ドメイン型は自分の表記を持たない**。`Meld` の `pon(5'55p,L)` も `RiverTile` の
  `4m+*` も構文は `Format/Notation/` の担当で、型の側には置かない。
  ロジックが表記に依存すると、フォーマットを変えたときに答えが変わってしまう
  （高点法の同点決着が MPSZ 文字列比較だった実績がある）。
  例外は `Tile` の MPSZ（`Format/` 以外にテストとCLIも使う牌のリテラル表記）と、
  `rawValue` が仕様の表記トークンである enum（`case 東 = "E"`）
