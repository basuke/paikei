...
- **Bool を返す述語は日本語で「〜か」**。プロパティも関数も同じ。
  ...
  （`Tile.赤か` は格納プロパティ）。
  一方、**値を保持するプロパティは英数字**（`suit` `rank` `honba` `kyotaku`）。
  Bool でも設定値なら英数字（`RuleSet.kuitan` `WinOptions.ippatsu`）— 「その卓が
  どうか」を問う述語ではなく、渡された設定だから
...
- **一括置換は事故る**。`.七対子` への置換が `Shanten.sevenPairs()` を、`.加槓` が
  `ClaimContext.kakan` を巻き込んだ実績がある。単語境界を付ける
  （`sed -E 's/\bisTerminal\b/老頭牌か/g'`）。置換後は
  `grep -rnE "(let|var) [^ -~]" Sources | grep -v "か: Bool"` で変数名に日本語が
  混ざっていないか確認する（`func` と Bool の述語は日本語で正しいので除く。
  テストは対象外）
