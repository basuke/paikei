...
struct 履歴からの導出 {
    ...
    @Test func 立直宣言より前の打牌は通った牌に入らない() throws {
        let t = try timeline(events: [
            .打牌(手番: .対面, 牌: try Tile.parse("1s"), ツモ切り: nil),   // 宣言前
            .ツモ(手番: .下家, 牌: nil),
            .立直(手番: .下家),
    ...
    @Test func 通った牌は安全度判定で現物になる() throws {
        var t = try timeline()
        t.snapshot.players[.下家] = PlayerState(riichi: true)
        t.events = [.打牌(手番: .対面, 牌: try Tile.parse("5s"), ツモ切り: nil)]
    ...
    @Test func 見逃した直後は同巡内フリテン() throws {
        ...
        let t = try timeline(events: [
            ...
            .打牌(手番: .対面, 牌: try Tile.parse("4p"), ツモ切り: nil),
            .ツモ(手番: .上家, 牌: nil),
        ])
    ...
    @Test func 待ちでない牌が通っても同巡内フリテンにならない() throws {
        let t = try timeline(events: [
            .ツモ(手番: .自分, 牌: try Tile.parse("9s")),
            .打牌(手番: .自分, 牌: try Tile.parse("9s"), ツモ切り: true),
            .打牌(手番: .対面, 牌: try Tile.parse("9s"), ツモ切り: nil),
        ])
        #expect(try !t.同巡内フリテン(of: .自分))
    }
    ...
    @Test func テンパイしていなければ同巡内フリテンではない() throws {
        ...
        t.events = [.打牌(手番: .対面, 牌: try Tile.parse("4p"), ツモ切り: nil)]
        #expect(try !t.同巡内フリテン(of: .自分))
    }
