...
struct Tileモデル {
    ...
    func 範囲外のrankは作れない() {
        #expect(Tile(suit: .萬子, rank: 0) == nil)
        #expect(Tile(suit: .萬子, rank: 10) == nil)
        #expect(Tile(suit: .字牌, rank: 7) != nil)
        #expect(Tile(suit: .字牌, rank: 8) == nil)
    }
    ...
    func 並び順はスート数値の順() {
        #expect(Tile(suit: .萬子, rank: 9)! < Tile(suit: .筒子, rank: 1)!)
        #expect(Tile(suit: .索子, rank: 9)! < Tile(suit: .字牌, rank: 1)!)
        #expect(Tile(suit: .筒子, rank: 3)! < Tile(suit: .筒子, rank: 4)!)
    }
...
struct 風と字牌の相互変換 {
    ...
    @Test("東=1z 〜 北=4z の対応")
    func 東1z北4zの対応() {
        #expect(Wind.東.tile == Tile(suit: .字牌, rank: 1))
        #expect(Wind.北.tile == Tile(suit: .字牌, rank: 4))
    }

    @Test("三元牌(5z〜7z)や数牌からは風にならない")
    func 三元牌や数牌からは風にならない() {
        #expect(Wind(tile: Tile(suit: .字牌, rank: 5)!) == nil)  // 白
        #expect(Wind(tile: Tile(suit: .萬子, rank: 1)!) == nil)
    }
}
...
struct MPSZパースシリアライズ {
    @Test func 基本の連結手牌をパースする() throws {
        ...
        #expect(tiles.first == Tile(suit: .萬子, rank: 1))
        #expect(tiles.last == Tile(suit: .字牌, rank: 1))
    }
